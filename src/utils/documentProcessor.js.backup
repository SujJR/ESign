...
const convertDocToDocx = async (docPath) => {
  try {
    ...
    // Create output path
    const baseName = path.basename(docPath, '.doc');
    const docxPath = path.join(path.dirname(docPath), `${baseName}.docx`);
...
const convertDocxToPdf = async (docxPath) => {
  try {
    ...
    const pdfPath = path.join(path.dirname(docxPath), `${baseName}.pdf`);
...
const extractTextContent = async (filePath) => {
  try {
    ...
    if (ext === '.docx') {
      ...
      // For PDF, we'll use the existing PDF analysis
      return 'PDF content extraction not implemented in this function';
    }
...
/**
 * Analyze document and determine signature fields from Adobe Sign
 * @param {string} filePath - Path to the document file
 * @returns {Promise<Object>} - Analysis result with signature fields
 */
const analyzeDocumentForSignatureFields = async (filePath) => {
  try {
    const textContent = await extractTextContent(filePath);
    const templateVariables = extractTemplateVariables(textContent);
    ...
    // Analyze template variables to categorize field types
    templateVariables.forEach(variable => {
      const lowerVar = variable.toLowerCase();
      
      if (lowerVar.includes('sign') || lowerVar.includes('signature')) {
        signatureFields.push(variable);
      } else if (lowerVar.includes('date')) {
        dateFields.push(variable);
      } else {
        textFields.push(variable);
      }
    });
    ...
    signaturePatterns.forEach(pattern => {
      if (pattern.test(textContent)) {
        logger.info(`Found signature pattern: ${pattern}`);
      }
    });
    ...
    const analysis = {
      ...
      dateFields,
      textFields,
      hasSignaturePatterns: signaturePatterns.some(pattern => pattern.test(textContent)),
      textContent: textContent.substring(0, 500) // First 500 chars for preview
    };
    
    logger.info(`Document analysis completed. Found ${signatureFields.length} signature fields, ${dateFields.length} date fields, ${textFields.length} text fields`);
    
    return analysis;
  } catch (error) {
    logger.error(`Error analyzing document for signature fields: ${error.message}`);
    throw error;
  }
};
...
/**
 * Converts DOC file to PDF (via DOCX conversion first if needed)
 * @param {string} docPath - Path to the DOC file
 * @returns {Promise<string>} - Path to the converted PDF file
 */
const convertDocToPdf = async (docPath) => {
  return new Promise((resolve, reject) => {
    try {
      logger.info(`Converting DOC to PDF: ${path.basename(docPath)}`);
      ...
      libre.convert(docBuffer, '.pdf', undefined, (err, pdfBuffer) => {
        if (err) {
          logger.error(`LibreOffice DOC conversion error: ${err.message}`);
          reject(new Error(`Failed to convert DOC to PDF: ${err.message}`));
          return;
        }

        try {
          // Generate PDF file path
          const originalName = path.basename(docPath, path.extname(docPath));
          const pdfFileName = `${originalName}_converted_${Date.now()}.pdf`;
          const pdfFilePath = path.join(path.dirname(docPath), pdfFileName);
          
          // Write the PDF file
          fs.writeFileSync(pdfFilePath, pdfBuffer);
          
          logger.info(`DOC to PDF conversion successful: ${pdfFilePath}`);
          resolve(pdfFilePath);
        } catch (writeError) {
          logger.error(`Error writing PDF file: ${writeError.message}`);
          reject(writeError);
        }
      });
      
    } catch (error) {
      logger.error(`Error in DOC to PDF conversion: ${error.message}`);
      reject(error);
    }
  });
};
