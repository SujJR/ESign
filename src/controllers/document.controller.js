...
exports.uploadDocumentFromUrl = async (req, res, next) => {
  try {
    ...
    const documentFile = await urlUtils.validateAndDownloadUrl(documentUrl);
    ...
    if (fileExtension === '.pdf') {
      ...
      if (Object.keys(templateData).length > 0) {
        try {
          ...
          const processedFilePath = await documentProcessor.processDocument(documentFile.path, templateData);
          documentData.processedFilePath = processedFilePath;
          
          // Convert to PDF
          const convertedFilePath = processedFilePath.replace('.docx', '_converted.pdf');
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        if (webhookUrl) {
          await adobeSign.createWebhook(accessToken, webhookUrl);
...
exports.sendReminder = async (req, res, next) => {
  try {
    ...
    // Get the Adobe Sign client
    const adobeSign = await createAdobeSignClient();
    const accessToken = await getAccessToken();
    
    // Get agreement details from Adobe Sign
    let agreementInfo;
    try {
      agreementInfo = await adobeSign.getAgreementInfo(accessToken, document.adobeAgreementId);
    } catch (error) {
    ...
    if (pendingRecipients.length === 0) {
      return res.status(200).json(formatResponse({
        message: 'No pending recipients to send reminder to',
        pendingRecipients: []
      }));
    }
    ...
    res.status(200).json(formatResponse({
      message: reminderSent 
        ? 'Reminder sent successfully via Adobe Sign' 
        : 'Reminder processing complete - returned pending recipients',
      pendingRecipients,
      reminderSent,
      reminderError,
      adobeSignInfo: {
  ...
  try {
    ...
    try {
      ...
      if (email) {
          ...
          for (const participantSet of agreementInfo.participantSets) {
            for (const participant of participantSet.memberInfos) {
              if (participant.email.toLowerCase() === email.toLowerCase()) {
                ...
                if (participant.status === 'SIGNED') {
                  return res.status(200).json(formatResponse({
                    message: 'Participant has already signed this document',
                    status: 'SIGNED',
                    email: participant.email
                  }));
                }
                ...
                const response = await adobeSign.getSigningUrl(
                  accessToken, 
                  document.adobeAgreementId,
        ...
        if (!signingUrl) {
          return res.status(200).json(formatResponse({
            message: 'No pending signers found. All participants have signed the document.',
            status: 'COMPLETED',
            document: {
              id: document._id,
              name: document.originalName,
...
exports.getAllSigningUrls = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return res.status(200).json(formatResponse({
        message: 'Signing URLs retrieved successfully',
        recipients,
        document: {
          id: document._id,
          name: document.originalName,
  ...
  try {
    ...
    if (document.status === 'signed' && document.adobeAgreementId && !document.signedDocumentPath) {
      try {
        ...
        const fileBuffer = await adobeSign.downloadSignedDocument(accessToken, document.adobeAgreementId);
